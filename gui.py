...
class gui_class:
    ...
    def __init__(self):
        ...
        self.cir_dim_mat = []
        self.custom_value = []
        # self.table2 = tablenoscroll.Table(self.root.frame5, ["Index", "Room Name"], column_minwidths=[None, None])
        # self.table2.pack(padx=10, pady=10)
        # self.table2.config(bg="#F4A5AE")
        ...
        while((self.value[0] == 0) and done):
            ...
            self.root.wait_variable(self.end)
            print("ender")
            if self.app.command != "custom" : 
                print("not custom")
                self.value=self.app.return_everything()
            else:
                self.value = self.custom_return
                        ...
                        if not i == j:
                            ...
                            if (leaf1.d3 == leaf2.d3 and leaf1.d4 == leaf2.d2):
                                ...
                                if leaf1.width <= leaf2.width:
                                    ...
                                    dim_mat[i,j] = leaf2.width
                                    dim_mat[j,i] = leaf2.width
                            if ((leaf1.d1 < leaf2.d1 and leaf1.d3 > leaf2.d1 and leaf1.d3 < leaf2.d3) and leaf1.d4 == leaf2.d2):
                                mat[i,j] = 1
                                mat[j,i] = 1
                                dim_mat[i,j] = (leaf1.d3-leaf2.d1)/scale
                                dim_mat[j,i] = (leaf1.d3-leaf2.d1)/scale
                            if ((leaf2.d1 < leaf1.d1 and leaf2.d3 > leaf1.d1 and leaf2.d3 < leaf1.d3) and leaf1.d4 == leaf2.d2):
                                mat[i,j] = 1
                                mat[j,i] = 1
                                dim_mat[i,j] = (leaf2.d3-leaf1.d1)/scale
                                dim_mat[j,i] = (leaf2.d3-leaf1.d1)/scale
                            if ((leaf1.d2 < leaf2.d2 and leaf1.d4 < leaf2.d4 and leaf2.d2 < leaf1.d4) and leaf1.d3 == leaf2.d1):
                                mat[i,j] = 1
                                mat[j,i] = 1
                                dim_mat[i,j] = (leaf1.d4-leaf2.d2)/scale
                                dim_mat[j,i] = (leaf1.d4-leaf2.d2)/scale
                            if ((leaf2.d2 < leaf1.d2 and leaf2.d4 < leaf1.d4 and leaf1.d2 < leaf2.d4) and leaf1.d3 == leaf2.d1):
                                mat[i,j] = 1
                                mat[j,i] = 1
    ...
    class Buttons:
        def __init__(self,root,master):
            ...
            b7 = tk.Button(master.frame1,width=15, text='Custom RFP' ,relief='flat',**button_details,command=master.custom)
            b7.grid(row=5,column=0,padx=5,pady=5)
    ...
    class output_text:
        def __init__(self,root):
            self.root=root
            self.textbox = tk.Text(root,bg=col[3],fg='black',relief='flat',height=25,width=30,padx=5,pady=5,**font)
    ...
    @property
    def custom_return(self):
        print("custom returner")
        self.custom_value = []
        rgraph = self.customapp.multi.final_graph
        # rgraph = self.customapp.value[1]

        self.custom_value.append(self.customapp.value[0])
        self.custom_value.append(len(rgraph))
        self.custom_value.append(rgraph)
        self.custom_value.append(self.command)
        self.custom_value.append(self.checkvar1.get())
        self.custom_value.append(self.customapp.value[2])
        self.custom_value.append(['#4BC0D9']* len(rgraph))
        self.custom_value.append(self.customapp.value[3])

        return self.custom_value
        # [len(self.nodes_data),self.edge_count,self.edges,self.command,self.master.checkvar1.get(),list(filter(None, [row[1].get() for row in self.table._data_vars])),self.hex_list]
    ...
    def custom(self):
        self.customapp = CustomRFPGui.CustomRFPPopup()
        self.customapp.start()
        self.app.command="custom"
        self.command = "custom"
        self.end.set(self.end.get()+1)
        self.end.set(self.end.get()+1)
        
        self.custom_table_head = tk.Label(self.frame5,text='Custom Table',bg =col[2])
        self.custom_table_head.grid(row = 2)
        
        self.custom_table = tablenoscroll.Table(self.frame5,["Code", "Room Name"], column_minwidths=[None, None])
        self.custom_table.grid(row = 3)
        ...
        self.custom_table.insert_row(["LR", "Living Room"], self.custom_table._number_of_rows)
        self.custom_table.insert_row(["MR", "Master Room"], self.custom_table._number_of_rows)
        self.custom_table.insert_row(["K", "Kitchen"], self.custom_table._number_of_rows)
        self.custom_table.insert_row(["BR", "Bathroom"], self.custom_table._number_of_rows)
        self.custom_table.insert_row(["DR", "Dining Room"], self.custom_table._number_of_rows)
        self.custom_table.insert_row(["CR", "Child Room"], self.custom_table._number_of_rows)
        self.custom_table.insert_row(["SR", "Study Room"], self.custom_table._number_of_rows)
        self.custom_table.insert_row(["2R", "Second Room"], self.custom_table._number_of_rows)
        self.custom_table.insert_row(["GR", "Guest Room"], self.custom_table._number_of_rows)
        self.custom_table.insert_row(["B", "Balcony"], self.custom_table._number_of_rows)
        self.custom_table.insert_row(["E", "Entrance"], self.custom_table._number_of_rows)
        self.custom_table.insert_row(["S", "Storage"], self.custom_table._number_of_rows)
